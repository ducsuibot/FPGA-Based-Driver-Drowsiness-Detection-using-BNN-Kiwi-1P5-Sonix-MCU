    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if is_dual:
                ...
                # Cấu trúc mới cho CONV2: 
                # [31:24] Neg Threshold (8 bit)
                # [23:16] Pos Threshold (8 bit)
                # [15]    Polarity (1 bit)
                # Các bit [14:0] để trống cho Weight
...
def pack_weights():
    ...
    with open(os.path.join(BASE_DIR, 'conv2_weight.txt'), 'r') as f:
        for i, line in enumerate([l.strip() for l in f if l.strip()]):
            for ch in range(16):
                # Chỉ lấy đúng 9 bit trọng số thật
                w_9bit = int(line[ch*9 : (ch+1)*9], 2) 
                
                if ch == 15:
                    # GỘP CHUNG: Threshold (Nửa trên) | Weight (9 bit dưới)
                    words_32bit.append(bn2[i] | w_9bit)
                else:
                    # Các kênh 0-14 giữ nguyên như cũ
                    w_val = int("1111111" + line[ch*9 : (ch+1)*9], 2)
                    words_32bit.append(w_val & 0xFFFFFFFF)

    # 3. Xử lý FC1 (64 neuron x 576 bit)
    with open(os.path.join(BASE_DIR, 'fc1_weight.txt'), 'r') as f:
        for i, line in enumerate([l.strip() for l in f if l.strip()]):
            for chunk_idx in range(36):
                ...
                # SỬA Ở ĐÂY: Rải Threshold vào TẤT CẢ các chunk
                t_val = bn3[i] 
                words_32bit.append((t_val << 16) | w_val)

    # 4. Xử lý FC2 (1 neuron x 64 bit)
    ...
    # =====================================================================
    # 1. XUẤT FILE CHUẨN INTEL HEX (DÙNG ĐỂ NẠP BẰNG QUARTUS XUỐNG CHIP)
    # =====================================================================
    hex_path = os.path.join(BASE_DIR, 'bnn_weights.hex')
    with open(hex_path, 'w') as f:
        address = 0
        # Ghi 4 Word (16 Bytes) mỗi dòng cho đẹp
        for i in range(0, len(words_32bit), 4):
            chunk = words_32bit[i:i+4]
            data_bytes = []
            for word in chunk:
                # ÉP KIỂU BIG-ENDIAN: Ghi Byte cao (MSB) xuống trước
                data_bytes.append((word >> 24) & 0xFF) # Byte 3
                data_bytes.append((word >> 16) & 0xFF) # Byte 2
                data_bytes.append((word >> 8) & 0xFF)  # Byte 1
                data_bytes.append(word & 0xFF)         # Byte 0

            record_len = len(data_bytes)
            hex_data = "".join(f"{b:02X}" for b in data_bytes)
            
            # Thuật toán tính Checksum bắt buộc của Intel HEX
            checksum_sum = record_len + (address >> 8) + (address & 0xFF) + 0 + sum(data_bytes)
            checksum = (~checksum_sum + 1) & 0xFF
            
            # Format dòng chuẩn: :[Len][Addr][Type(00)][Data][Checksum]
            f.write(f":{record_len:02X}{address:04X}00{hex_data}{checksum:02X}\n")
            address += record_len
            
        f.write(":00000001FF\n") # Dòng báo hiệu kết thúc file (EOF)
        
    # =====================================================================
    # 2. XUẤT FILE RAW HEX (DÀNH CHO LỆNH $readmemh TRONG MODELSIM)
    # =====================================================================
    sim_path = os.path.join(BASE_DIR, 'bnn_weights_sim.txt')
    with open(sim_path, 'w') as f_sim:
        for word in words_32bit:
            # Ghi thuần túy 8 ký tự Hex (32-bit), mỗi dòng 1 word, không có tiền tố hay checksum
            f_sim.write(f"{word:08X}\n")

    print(f"Đã đóng gói {len(words_32bit)} words thành công!")
    print(f" -> File nạp mạch : bnn_weights.hex (Intel HEX, Big-Endian)")
    print(f" -> File mô phỏng : bnn_weights_sim.txt (Raw HEX 32-bit)")
