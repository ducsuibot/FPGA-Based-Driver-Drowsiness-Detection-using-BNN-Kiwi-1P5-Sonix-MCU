...
int w_conv1[16][9], w_conv2[16][144], w_fc1[32][576], w_fc2[32]; // Sửa w_fc1 64->32, w_fc2 64->32
int bn1_p[16], bn1_t[16];
int bn2_p[16], bn2_tpos[16], bn2_tneg[16];
int bn3_p[32], bn3_t[32]; // Sửa bn3 64->32
int bn4_p, bn4_t;
...
int main() {
    ...
    read_mem("fpga_weights/fc1_weight.mem", (int*)w_fc1, 18432); // 32 * 576 = 18432
    read_mem("fpga_weights/fc2_weight.mem", w_fc2, 32);          // 64 -> 32
    ...
    f = fopen("fpga_weights/bn3_thresholds.txt", "r"); for(int i=0; i<32; i++) fscanf(f, "%d %d", &bn3_p[i], &bn3_t[i]); fclose(f); // i<64 -> i<32
    ...
    printf("# FINAL DECISION | Popcount: %d/32 | Threshold: %d -> RESULT: %d\n#\n", pop_final, bn4_t, result); // 64 -> 32
    printf("# ========================================\n");
