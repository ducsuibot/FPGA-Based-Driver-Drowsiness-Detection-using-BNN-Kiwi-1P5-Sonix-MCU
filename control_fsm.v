        ...
        wt_addr <= pixel_cnt + 2; // thay vì pixel_cnt + 1, pre-fetch trước 1
                ...
                WAIT_SYNC: begin 
                    if (wait_cnt == 3) begin 
                        ...
                        if (next_state_save == FC1) begin 
                            act_rd_addr <= 936; 
                            wt_addr     <= 272; // đây là pre-fetch cho FC1 cycle đầu
                        end
...
FC1: begin
    ...
    // Quản lý wt_addr:
    // pixel_cnt=0: KHÔNG SET gì → wt_addr giữ = 272 (set từ WAIT_SYNC)
    // pixel_cnt=1..35: tăng để pre-fetch cho cycle tiếp theo
    // pixel_cnt=36: không cần set (wt_data đã có sẵn từ cycle trước)
    if (pixel_cnt >= 1 && pixel_cnt <= 35) begin
        wt_addr <= wt_addr + 1;
    end else if (pixel_cnt == 0 && out_ch > 0) begin
        wt_addr <= wt_addr + 1;
    end
    // out_ch=0, pixel_cnt=0: không làm gì ← đây là điểm mấu chốt
    ...
    // pixel_cnt=0: KHÔNG SET wt_addr → giữ nguyên 2576 từ WAIT_SYNC
    // pixel_cnt=1..3: tăng để pre-fetch cho cycle tiếp theo
    // pixel_cnt=4: không cần (wt_data đã sẵn)
    if (pixel_cnt >= 1 && pixel_cnt <= 3) begin
        wt_addr <= wt_addr + 1;
    end
