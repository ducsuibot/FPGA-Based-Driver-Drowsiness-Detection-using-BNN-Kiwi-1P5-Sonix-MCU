    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // TRONG QUÁ TRÌNH MCU ĐANG GỬI DỮ LIỆU
                ...
                if (rx_valid) rx_addr <= rx_addr + 1'b1;
            ...
            // KHI MCU KÉO CS LÊN KẾT THÚC GỬI ẢNH
            else if (spi_cs_rise) begin
                // FIX BUG-I: Nếu byte cuối cùng đang chốt (rx_valid) và addr = 71
                // hoặc addr đã nhảy lên 72, ta kích hoạt frame_done.
                if (rx_addr == TOTAL_BYTES || (rx_addr == TOTAL_BYTES - 1 && rx_valid)) begin 
                    frame_done <= 1'b1;
                end
            end
            ...
            // KHI MCU KÉO CS LÊN KẾT THÚC GỬI ẢNH
                ...
                // FIX LỖI 2: Check >= 71 để bù trừ clock cycle
