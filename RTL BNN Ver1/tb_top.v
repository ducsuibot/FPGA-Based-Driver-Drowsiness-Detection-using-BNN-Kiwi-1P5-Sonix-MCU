...
module tb_top();
    ...
    // =========================================================================
    // 3. LOGIC IN BÁO CÁO (CHỈ HIỆN LAYER 3 VÀ LAYER 4)
    // =========================================================================
    ...
    always @(negedge clk) begin
        if (uut.u_fsm.is_fc && uut.u_datapath.valid_out) begin
            ...
            // --- LAYER 3: FC1 (Từ Node 0 đến Node 63) ---
            if (fc_pulse_cnt < 64) begin
                if (fc_pulse_cnt == 0)
                    $display("# >>> LAYER 3: FULLY CONNECTED 1 (Toan bo 64 Node) <<<");
            ...
            else if (fc_pulse_cnt == 64) begin
                $display("# \n# >>> LAYER 4: FULLY CONNECTED 2 (Lop chot ha) <<<");
                $display("# FINAL DECISION | Popcount: %0d/64 | Threshold: %0d -> RESULT: %b",
    ...
    initial begin
        ...
        // Gửi SPI im lặng (không in log rườm rà)
        spi_img_cs = 0;
        for (i_byte = 0; i_byte < 72; i_byte = i_byte + 1) begin
        ...
        $display("# => KET LUAN CUA PHAN CUNG: %s (%b)", (result ? "MO MAT" : "NHAM MAT"), result);
        $display("# ========================================");
        $finish;
    end
