...
module tb_pool2;
    ...
    // Đón dữ liệu Golden của POOL2 (6x6 = 36 pixels)
    reg [15:0] golden_mem [0:35]; 
    ...
    initial begin
        ...
        $readmemh("golden_output_p2.txt", golden_mem); // Load mảng POOL2
        ...
        @(posedge clk);
        spi_we = 0;
        $display("Nap hoan tat! Bat dau chay C1 -> P1 -> C2 -> P2...");
        $display("----------------------------------------");
        ...
        #2100000; // Timeout vẫn để dài vì phải đợi CONV2 chạy xong
        $display("TIMEOUT: Mo phong bi treo hoac dien ra qua lau.");
        $stop;
    end

    // --- LOGIC ĐÓN LÕNG KẾT QUẢ POOL2 ---
    integer match_cnt = 0;
    integer err_cnt = 0;
    integer out_idx = 0;

    always @(posedge clk) begin
        // Dải địa chỉ POOL2 nằm từ 936 đến 971 (36 pixels)
        if (act_we && (act_wr_addr >= 936) && (act_wr_addr < 972)) begin
            
            out_idx = act_wr_addr - 936;
            
            if (act_wr_data === golden_mem[out_idx]) begin
                match_cnt = match_cnt + 1;
            end else begin
                err_cnt = err_cnt + 1;
                $display("[FAIL] POOL2 Pixel %0d | FPGA: %04X | Golden: %04X", out_idx, act_wr_data, golden_mem[out_idx]);
            end

            // Kết thúc khi so sánh đủ 36 pixel
            if (out_idx >= 35) begin
                #10;
                $display("\n========================================");
                $display("         KET QUA TEST POOL 2            ");
                $display("========================================");
                $display("Tong so Pixel da so sanh: 36");
                $display("So luong MATCH (Khop)   : %0d", match_cnt);
                $display("So luong LOI            : %0d", err_cnt);
                if (err_cnt == 0) begin
                    $display(">>> TUYET VOI! POOL 2 chay chuan xac! <<<");
                end else begin
                    $display(">>> THAT BAI! Kiem tra lai logic POOL2. <<<");
                end
                $display("========================================\n");
                $finish;
            end
