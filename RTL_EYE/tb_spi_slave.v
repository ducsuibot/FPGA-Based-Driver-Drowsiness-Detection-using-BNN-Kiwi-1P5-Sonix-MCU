...
module tb_spi_slave();
    ...
    // ==========================================
    // MÓC TÍN HIỆU NỘI BỘ (DEBUG PROBE)
    // Kéo biến bit_cnt từ bên trong u_spi ra ngoài để xem Waveform
    // ==========================================
    wire [2:0] debug_bit_cnt = u_spi.bit_cnt;
    ...
    padding_and_buffer u_pad (
        ...
        // Tắt các tín hiệu điều khiển mạng AI (chỉ test nạp ảnh)
    ...
    // ==========================================
    // 6. TASK GIẢ LẬP GIAO THỨC SPI CỦA MCU
    // ==========================================
    task send_spi_byte(input [7:0] data);
        ...
        begin
            for (b = 7; b >= 0; b = b - 1) begin
                ...
                // Delay 1 chút trước khi kéo sườn dương (Setup Time)
                #40; 
                spi_clk = 1;            // Sườn dương: FPGA sẽ chốt bit ở đây
                
                // Giữ mức cao một lúc rồi kéo xuống (Hold Time)
                #40; 
                spi_clk = 0;        
            end
        end
    ...
    initial begin
        ...
        for (i = 0; i < 72; i = i + 1) begin
            ...
            // Theo dõi trên console mỗi khi đẩy xong 1 byte
            $display("[%0t] MCU gui byte %2d: %b", $time, i, spi_tx_buffer[i]);
            #100; 
        end
        ...
        for (i = 0; i < 72; i = i + 1) begin
            if (u_pad.act_ram[i][7:0] !== spi_tx_buffer[i]) begin
                $display(">>> [LOI] BRAM[%2d] = %b | MCU = %b <<<", i, u_pad.act_ram[i][7:0], spi_tx_buffer[i]);
            end
        end
        
        $display("\n==================================================");
        $display(" HOAN TAT TEST! (Tat ca du lieu khop 100)");
        $display("==================================================\n");
