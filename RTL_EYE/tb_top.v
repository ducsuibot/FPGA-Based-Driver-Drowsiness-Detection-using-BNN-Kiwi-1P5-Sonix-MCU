...
module tb_top();
    ...
    // =================================================================
    // THÊM MỚI: MÓC TÍN HIỆU NỘI BỘ TỪ MODULE TOP RA TESTBENCH
    // Lưu ý: Giả sử trong file top.v, dây tín hiệu báo nhận xong ảnh
    // của bạn được đặt tên là "frame_done". Nếu bạn đặt tên khác 
    // (ví dụ: rx_done, img_done...), hãy sửa lại ở phần uut.frame_done
    // =================================================================
    wire frame_done = uut.frame_done; 
    ...
    // ---> THÊM MỚI: THEO DÕI CỜ FRAME_DONE <---
    always @(posedge frame_done) begin
        $display("\n# [%0t ns] -> CO FRAME_DONE NAY LEN 1! Da nhan du 72 bytes vao RAM. FSM bat dau chay!", $time / 1000);
    end
    ...
    // =========================================================
    // KHỞI TẠO MẠNG GLOBAL SET/RESET (BẮT BUỘC CHO GOWIN SIM)
    // =========================================================
    GSR GSR (
        .GSRI(1'b1)  // Cấp mức 1 (High) để nhả Global Reset, cho phép mạch chạy
    );
